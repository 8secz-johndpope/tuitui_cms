...
router.get('/', async(req, res, next) => {
    ...
    let { type = "manual", page } = req.query;
    let messages = [];
    if(!page) {
        if (type === "is_timing") {
...
router.get('/getByType', async (req, res, next) => {
    ...
    switch (type) {
        case "is_timing":
            ...
            break;
        case "delay":
            messages = await MessageModel.find({account_id, delay: {$elemMatch:{$lte: 0}}}).skip((page - 1) * 10).limit(10).sort({
                _id: -1, codes: 1
            });
            total = await MessageModel.count({account_id, delay: {$elemMatch:{$lte: 0}}})
            break;
        case "is_daily":
